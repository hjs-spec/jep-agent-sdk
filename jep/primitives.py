...
from jep.core.event import build_event, _compute_what
import json
...
def judge(who: str, content: Any = None, what: Optional[str] = None, **kwargs) -> Dict[str, Any]:
    if what is None and content is not None:
        what = _content_to_what(content)
    return build_event("J", who, what=what, **kwargs)


def delegate(who: str, content: Any = None, what: Optional[str] = None, **kwargs) -> Dict[str, Any]:
    if what is None and content is not None:
        what = _content_to_what(content)
    return build_event("D", who, what=what, **kwargs)


def terminate(who: str, content: Any = None, what: Optional[str] = None, **kwargs) -> Dict[str, Any]:
    if what is None and content is not None:
        what = _content_to_what(content)
    return build_event("T", who, what=what, **kwargs)


def verify(who: str, ref: str, content: Any = None, what: Optional[str] = None, **kwargs) -> Dict[str, Any]:
    if what is None and content is not None:
        what = _content_to_what(content)
