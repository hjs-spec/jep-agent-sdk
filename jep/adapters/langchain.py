...
Usage:
    import jep.adapters.langchain.auto   # <-- That's it. Every AgentExecutor now records JEP.
"""
...
class _JEPCallbackHandler:
    """Minimal callback interface compatible with LangChain's BaseCallbackHandler."""
    
    def __init__(self, issuer: str = "langchain:agent", private_key=None, storage_path: Optional[str] = None):
        self.chain = AuditChain(issuer=issuer, private_key=private_key, storage_path=storage_path)
        self._run_stack: List[str] = []
    
    def on_chain_start(self, serialized: Optional[Dict[str, Any]] = None, inputs: Optional[Dict[str, Any]] = None, **kwargs: Any) -> Any:
    ...
    def on_tool_start(self, serialized: Optional[Dict[str, Any]] = None, input_str: Optional[str] = None, **kwargs: Any) -> Any:
        tool_name = serialized.get("name") if serialized else "unknown"
        content = {"type": "tool_start", "tool": tool_name, "input": input_str}
        ev = judge(who=self.chain.issuer, content=content)
        self.chain.append(ev)
    
    def on_tool_end(self, output: Optional[str] = None, observation: Optional[str] = None, **kwargs: Any) -> Any:
        content = {"type": "tool_end", "output": str(output) if output else str(observation)}
        ev = verify(who=self.chain.issuer, content=content)
        self.chain.append(ev)
...
def enable_tracing(issuer: str = "langchain:agent", private_key=None, storage_path: Optional[str] = None):
...
# Auto-patch module: import jep.adapters.langchain.auto
